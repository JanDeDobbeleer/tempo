...
const entries: Entry[] = [
  { id: 'e1', date: '2026-01-05', projectId: 'p1', start: 540, end: 1020, comment: '', attachments: [] }, // 8h @800/8h = 800
  { id: 'e2', date: '2026-01-10', projectId: 'p2', start: 540, end: 780, comment: '', attachments: [] }, // 4h @600/8h = 300
  { id: 'e3', date: '2026-02-01', projectId: 'p3', start: 540, end: 1020, comment: '', attachments: [] }, // 8h @400/8h = 400
];
...
describe('earnings helpers', () => {
  test('entryEarnValue applies the day-rate formula and 0 for a missing project', () => {
    expect(entryEarnValue(entries[0], projects[0], HOURS_PER_DAY)).toBeCloseTo(800);
    expect(entryEarnValue(entries[1], projects[1], HOURS_PER_DAY)).toBeCloseTo(300);
    expect(entryEarnValue(entries[0], undefined, HOURS_PER_DAY)).toBe(0);
  });

  test('filterEntries applies inclusive date bounds', () => {
    const result = filterEntries(entries, projects, { fromISO: '2026-01-01', toISO: '2026-01-31', customerId: null, projectId: null });
    expect(result.map((entry) => entry.id)).toEqual(['e1', 'e2']);
  });

  test('filterEntries AND-composes customer and project filters', () => {
    const byCustomer = filterEntries(entries, projects, { fromISO: '2026-01-01', toISO: '2026-12-31', customerId: 'c1', projectId: null });
    expect(byCustomer.map((entry) => entry.id)).toEqual(['e1', 'e2']);

    const byProject = filterEntries(entries, projects, { fromISO: '2026-01-01', toISO: '2026-12-31', customerId: 'c1', projectId: 'p2' });
    expect(byProject.map((entry) => entry.id)).toEqual(['e2']);
  });

  test('filterEntries treats null customer/project as "all"', () => {
    const result = filterEntries(entries, projects, { fromISO: '2026-01-01', toISO: '2026-12-31', customerId: null, projectId: null });
    expect(result).toHaveLength(3);
  });

  test('summarize totals minutes, earnings, days and entry count', () => {
    const projById = { p1: projects[0], p2: projects[1], p3: projects[2] };
    const summary = summarize(entries, projById, HOURS_PER_DAY);
    expect(summary.count).toBe(3);
    expect(summary.minutes).toBe((1020 - 540) + (780 - 540) + (1020 - 540));
    expect(summary.earn).toBeCloseTo(800 + 300 + 400);
  ...
  test('aggregateBy groups by customer, sorts by earnings desc, and shares sum to ~100', () => {
    const rows = aggregateBy(entries, projects, customers, 'customer', HOURS_PER_DAY);
    expect(rows.map((row) => row.id)).toEqual(['c1', 'c2']);
    expect(rows[0].earn).toBeCloseTo(1100);
    expect(rows[1].earn).toBeCloseTo(400);
    const totalShare = rows.reduce((sum, row) => sum + row.sharePct, 0);
    expect(totalShare).toBeCloseTo(100);
  });

  test('aggregateBy groups by project', () => {
    const rows = aggregateBy(entries, projects, customers, 'project', HOURS_PER_DAY);
  ...
  test('aggregateBy returns 0 shares when total earnings is 0', () => {
    ...
    const zeroEntries: Entry[] = [{ id: 'e0', date: '2026-01-05', projectId: 'p0', start: 540, end: 1020, comment: '', attachments: [] }];
